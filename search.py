...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    if isinstance(problem, searchAgents.CornersProblem):
        fringe.push([[startState], 0, startHeur], startHeur) # [ [(state-tuple), (state-tuple), ...], 0, ] -> not entirely sure what the 0 and startHeur are that are inside the list :)))))
    else:
    ...
    while not fringe.isEmpty():
        curr = fringe.pop()  # this is of the format [[(state, dir, cost), ... , (state, dir, cost)], totalPathCost]
        mostRecentState = curr[0][-1][0]
        if problem.isGoalState(mostRecentState):
            path = curr[0]
            break
        elif mostRecentState not in closed:
        # elif curr[-1] not in closed:

            closed.append(mostRecentState)
            # print("mostRecentState: ", mostRecentState)
            # print("curr: ", curr)
            if isinstance(problem, searchAgents.CornersProblem):
        ...
        #     elif curr[-1] not in closed:
        #         closed.append(curr[-1])
        #         succs = problem.getSuccessors(curr[0][-1])
        #         print("succs: ", succs)
        #         succsReverse = succs[::-1]
        #         print("succsReverse: ", succsReverse)
        #         for succ in succs:
        #             print("succ in for: ", succ)
        #             potentialPath = curr[0] + succ
        #             potentialCost = curr[1] + succ[2]
        #             potentialHeur = heuristic(succ, problem)
        #             potential = [potentialPath, potentialCost, potentialHeur]
        #             fringe.push(potential, potentialCost + potentialHeur)
        # else:
        #     if problem.isGoalState(curr[0][-1][0]):
